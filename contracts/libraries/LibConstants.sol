...
library LibConstants {
	...
	function base() internal view returns (uint64) {
		return BASE_VALUE;
	}

	function bronze() internal view returns (uint64) {
		return BRONZE_VALUE;
	}

	function silver() internal view returns (uint64) {
		return SILVER_VALUE;
	}

	function gold() internal view returns (uint64) {
		return GOLD_VALUE;
	}
}
